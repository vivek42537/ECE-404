...
def sha512(input_hash,output_hash):
    ...
    #convert Kval to a bit vector
    K_bv = [BitVector(hexstring = k_constant) for k_constant in K]

    #step1 from lecture slides
    bv = BitVector(textstring = input_hash)
    bv1 = bv + BitVector(bitstring="1")
    howmanyzeros = (896 - bv1.length()) % 1024
    ...
    with open(output_hash, 'w') as file_out:
        file_out.write(hash_hex_string) 

with open(sys.argv[1]) as file_in:
    input_hash = file_in.read()

sha512(input_hash,sys.argv[2])